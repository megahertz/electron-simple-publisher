...
class GithubApi {
  ...
  request(route, requestData, reqOptions = {}) {
    const options = this.normalizeOptions(route, requestData, reqOptions);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (typeof requestData.pipe === 'function') {
  ...
  normalizeOptions(route, data = {}, reqOptions = {}) {
    ...
    if (typeof options.data.pipe !== 'function') {
      for (const field in options.data) {
        if (!options.data.hasOwnProperty(field)) continue;
        if (field.startsWith('_')) {
          urlParams[field.substring(1)] = options.data[field];
          delete options.data[field];
        }
      }
    }
