...
function addAssetsInfo(build) {
  ...
  const assetsPath = path.resolve(meta.dir);
...
function replaceMaskByPath(assets, files, dirPath) {
  ...
  return keys.reduce((result, name) => {
    ...
    if (fileName) {
      ...
      throw new Error(`File with mask ${mask} is not found at ${dirPath}`);
    }
    return result;
  }, {});
}